...
- 2026-09-03 - CORRECTION to the entry above. The cause is not the cache key. Measured: apex and www hold separate objects for the same path (Age 1 vs Age 1146), so the key does carry the host. The real mechanism is Workers Assets answering before the Worker runs - on www a prerendered path returns 200 HIT while /api/health/ and a nonexistent path both return 301, which pins it. One mechanism, both symptoms: redirectToCanonicalHost never runs for a real page, and a zone purge addresses a cache that is not the one serving. Purge ineffectiveness re-confirmed on a second URL (/robux-to-usd/, Age 952 to 955 across eight requests after success:true). A cache rule does not fix either half; www wants a Redirect Rule, which needs Zone > Config Rules.
- 2026-09-03 - GA4 enabled at the owner's instruction (G-B776T5VD3W), and the four statements it would have falsified rewritten in all seven languages: the absolute 'loads no third-party scripts' is now the specific, true claim about a market-data vendor's script. Four tests moved from asserting this deployment's configuration to asserting the invariant - analytics load iff /privacy/ names GA4, a CSP origin is allowed iff the page uses it, /platform/ and /platform/stock/ reach no market-data host rather than no host. Deployed b3c4743f, verified live: tag on every page, CSP allows it, /privacy/ names GA4, stock page reworded, 7/7 production tests pass.

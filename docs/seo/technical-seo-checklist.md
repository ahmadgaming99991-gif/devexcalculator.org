...
- [ ] **B4** — Side effect of B3, and a large one: converting the 8 English `opengraph-image.tsx` routes and `apple-icon.tsx` to committed PNGs removes `@vercel/og` from the Worker. Bundle was at **92% of the 3 MB limit** — documented as blocking debt.

...
export function ThemeToggle({ words }: { readonly words: LocaleWords }) {
  ...
  return (
    <button
      type="button"
      onClick={() => apply(next)}
      className="inline-flex size-[44px] items-center justify-center rounded-(--radius-control) border border-(--color-border) text-(--color-text-muted) hover:text-(--color-text)"
      title={label}
    >
